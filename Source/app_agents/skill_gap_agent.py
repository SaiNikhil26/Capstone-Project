...
class SkillGapAgent:
    ...
    async def analyse(self, level: str, courses: list[dict]) -> SkillGapResult:
        log.info("[SkillGapAgent:analyse] START | level=%r | courses=%d", level, len(courses))

        prompt = self._build_prompt(level, courses)
    ...
    def _build_prompt(self, level: str, courses: list[dict]) -> str:
        lines = [f"Student level: {level}\n", "Recommended courses:"]
